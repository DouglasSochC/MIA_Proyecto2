...
router.post('/addTecnico', async (req, res) => {
    const { nombre, fecha_nacimiento, link_fotografia, id_pais, id_estado_tecnico, id_equipo, fecha_inicial } = req.body;
    if (!(nombre != "" && fecha_nacimiento != "" && id_pais != -1 && id_estado_tecnico != -1 && id_equipo != -1 && fecha_inicial != "")) {
        res.status(201).json({
            "response":false,
        ...
        if (r_existencia.rows.length > 0) {
            ...
            let result = await BD.Open(sql, [nombre, fecha_nacimiento, link_fotografia, id_pais, id_estado_tecnico], true);
            if (result.rowsAffected) {
                sql_trayectoria = "INSERT INTO TRAYECTORIA_TECNICO(id_tecnico, id_equipo, fecha_inicial) VALUES(\
                    (SELECT id FROM TECNICO WHERE nombres = :nombre AND id_nacionalidad = :id_pais AND id_estado = :id_estado),\
                    :id_equipo,\
                    TO_DATE(:fecha_inicial, 'dd/mm/yyyy')\
                    )"   
                    await BD.Open(sql_trayectoria, [nombre, id_pais, id_estado_tecnico, id_equipo, fecha_inicial], true);
            }
            res.status(201).json({
                "response":true,
